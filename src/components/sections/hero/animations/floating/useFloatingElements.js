...
export function useFloatingElements(containerRef) {
  ...
  useGSAP(() => {
    ...
    // Return cleanup for manual event listeners (GSAP animations cleaned up automatically)
    return () => {
      // Only cleanup non-GSAP elements manually
      cleanupAnimations(animationsRef);
    };
  }, { scope: containerRef, dependencies: [setupAnimations] });
