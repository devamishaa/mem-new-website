...
import ScalingText from "@/app/components/common/scalingText/ScalingText";
import BlurryCursor from "@/app/components/common/BlurryCursor";
...
export default function FooterView({ model }) {
  ...
  // Removed breakpoint logic - using fixed responsive design

  return (
    <footer
      className="flex w-full flex-col rounded-t-[32px] border-t border-gray-200 bg-[#eaf2f1] px-4 pt-4 text-[#01214f]"
      ref={footerRef}
      data-footer
    >
      <div className="mx-auto w-full" data-footer-container>
        <div
          className="mb-8 flex items-center justify-between max-md:mb-0 max-md:flex-col max-md:gap-4 max-md:text-center lg:mb-12 lg:px-1"
          data-footer-top
        >
          <div className="footer-text" data-footer-text>
            <p className="m-0 font-normal leading-snug text-[#898f91] text-2xl max-md:text-lg max-sm:text-base lg:text-3xl">
          ...
          <div
            className="flex items-center gap-1 max-md:justify-center max-md:gap-3 lg:gap-3"
            data-footer-social
          >
            <Link
              href="https://www.linkedin.com/company/memorae-ai/"
              aria-label="LinkedIn"
              className="group flex items-center justify-center rounded-lg p-2 text-black transition-all duration-300 ease-in-out hover:-translate-y-0.5 hover:text-[#5f64ff] max-sm:p-1.5"
              data-footer-social-icon
              target="_blank"
              rel="noopener noreferrer"
            >
              <svg
                className="h-6 w-6 fill-current max-sm:h-5 max-sm:w-5"
      ...
      <div
        ...
        style={{
          minHeight: `${Math.max(50, scalingTextHeight)}px`,
      ...
      <div
        className="hidden" /* This corresponds to .blurryCursorMobile */
        data-footer-mobile-cursor
      ></div>

      <div
        className="mt-auto flex flex-wrap items-center justify-between gap-4 border-t border-gray-200 py-4 max-md:flex-col max-md:gap-6 max-md:text-center lg:p-8"
        data-footer-bottom
      >
        <p className="m-0 text-sm text-[#898f91] max-md:hidden">
          {model?.footer?.copyright ||
            "© 2024 Gemera Capital SLU. All rights reserved."}
        </p>

        <div
          className="flex flex-wrap items-center gap-6 max-md:order-2 max-md:flex-col max-md:gap-4"
          data-footer-links
        >
          <Link
            href="https://memorae.ai/privacy-policy"
            className="text-sm font-normal text-[#898f91] underline transition-colors duration-200 ease-in-out hover:text-[#01214f] max-sm:text-xs"
            target="_blank"
            rel="noopener noreferrer"
          >
        ...
        <p className="m-0 text-sm text-[#898f91] md:hidden order-3 max-sm:text-xs">
          {model?.footer?.copyright ||
            "© 2024 Gemera Capital SLU. All rights reserved."}
        </p>
      </div>
