...
function splitTextIntoWords(element) {
  ...
  // Use a more reliable method to replace content
  element.innerHTML = "";
...
export function useTextReveal(containerRef) {
  ...
  useLayoutEffect(() => {
    ...
    // Create timeline with optimized animations
    const timeline = gsap.timeline({ paused: true });

    processedWordSpans.forEach(({ wordSpans, elementIndex }) => {
      timeline.to(wordSpans, textAnimationConfig, elementIndex * 0.1); // Delay between different text elements
    });
  ...
  }, [containerRef, textAnimationConfig]);
